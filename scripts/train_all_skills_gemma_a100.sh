#!/bin/bash
# ============================================================
# Full Pipeline: Download Data → Prepare Skill Datasets → Train (Gemma 2 9B)
# ============================================================
#
# Self-contained script for 1x A100 80GB. Uses the full-finetuned
# Gemma 2 9B model as the base and trains all 5 skill LoRA adapters
# sequentially.
#
# Usage:
#    bash scripts/train_all_skills_gemma_a100.sh
#    bash scripts/train_all_skills_gemma_a100.sh --skills "cbt-therapy psychoeducation"
#    bash scripts/train_all_skills_gemma_a100.sh --skip-download   # datasets already exist
#    bash scripts/train_all_skills_gemma_a100.sh --skip-train      # only download + prepare
#    bash scripts/train_all_skills_gemma_a100.sh --base-model /path/to/base/model
#
# ============================================================
...
BASE_MODEL="models/gemma2-9b-mental-health-fullft-a100"
ADAPTER_PREFIX="adapters-gemma"
SKIP_DOWNLOAD=false
SKIP_TRAIN=false
CLEAN_FAILED=false
...
echo "========================================================"
echo "  Skill LoRA Training Pipeline — Gemma 2 9B (A100 80GB)"
echo "  Start: $(date)"
echo "========================================================"
...
if [[ "$BASE_MODEL" == */* && "$BASE_MODEL" != /* ]]; then
    # Looks like a HuggingFace model ID (e.g. google/gemma-2-9b-it)
    ...
    echo "       bash $0 --base-model google/gemma-2-9b-it"
    echo "  2. Copy your fine-tuned model:"
    echo "       scp -r vm:~/LLM/models/gemma2-9b-mental-health-fullft-a100 $PROJECT_DIR/models/"
    echo ""
    exit 1
else
    echo "Base model (local): $RESOLVED_MODEL"
...
echo ""
echo "========================================================"
echo "  PHASE 3: Train LoRA Adapters — Gemma 2 9B (1x A100 80GB)"
echo "========================================================"
echo ""
echo "  NOTE: Gemma 2 9B is memory-tight on A100 80GB."
echo "  Using conservative settings: batch_size=2, max_length=512"
echo ""
...
for SKILL in $SKILLS; do
    ...
    # Gemma 2 requires tighter memory settings
...
echo ""
echo "========================================================"
echo "  Pipeline Complete — Gemma 2 9B Skill Adapters"
echo "  End: $(date)"
echo "========================================================"
echo "  Total:   $TOTAL"
