#!/bin/bash
# ============================================================
# Full Pipeline: Download Data → Prepare Skill Datasets → Train
# ============================================================
#
# Self-contained script for 1x A100 80GB. Downloads all source
# datasets from HuggingFace/GitHub/Kaggle, processes them into
# skill-specific splits, then trains all 5 remaining LoRA
# adapters sequentially.
#
# Usage:
#    bash scripts/train_all_skills_a100.sh
#    bash scripts/train_all_skills_a100.sh --skills "cbt-therapy psychoeducation"
#    bash scripts/train_all_skills_a100.sh --skip-download   # datasets already exist
#    bash scripts/train_all_skills_a100.sh --skip-train      # only download + prepare
#    bash scripts/train_all_skills_a100.sh --base-model /path/to/base/model
#
# ============================================================
...
echo "========================================================"
echo "  Skill LoRA Training Pipeline (A100 80GB)"
echo "  Start: $(date)"
echo "========================================================"
...
if [ "$SKIP_TRAIN" = true ]; then
    ...
    echo "  python scripts/train_skill_lora.py --skill <skill_name>"
    exit 0
fi
...
echo ""
echo "========================================================"
echo "  PHASE 3: Train LoRA Adapters (1x A100 80GB)"
echo "========================================================"
...
for SKILL in $SKILLS; do
    ...
    CMD="uv run python3 scripts/train_skill_lora.py --skill $SKILL --base_model $RESOLVED_MODEL"
    if [ -f "$CONFIG" ]; then
